  ...
  useEffect(() => {
    const fetchAppointments = async () => {
      try {
        ...
        const serviceOrders = data.filter(order => order.orderType === 'Service');
        setAppointments(serviceOrders);
      } catch (err) {
                ...
                {filteredAppointments.map((appt) => (
                  <tr key={appt._id}>
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">
                      #{appt._id.substring(0, 8)}...
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-600">
                      {appt.customer?.name || 'Unknown'}
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-600">
                      {/* Show first service name, or 'Multiple Services' */}
                      {appt.orderItems.length > 0 ? appt.orderItems[0].name : 'Service'}
                      {appt.orderItems.length > 1 && <span className="text-xs text-gray-400 ml-1">(+{appt.orderItems.length - 1})</span>}
                    </td>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                      <Link 
                        to={`/shop/dashboard/appointments/${appt._id}`}
